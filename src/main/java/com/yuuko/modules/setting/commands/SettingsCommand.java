...
public class SettingsCommand extends Command {
    ...
    public void onCommand(MessageEvent e) throws Exception {
        ...
        HashMap<String, String> settings = GuildFunctions.getGuildSettings(e.getGuild().getId());

        String starboard = "`Disabled` - React with a ⭐ to `star` messages and send them to this channel.";
        if(settings.get("starboard") != null) {
            TextChannel textChannel = e.getGuild().getTextChannelById(settings.get("starboard"));
        ...
        if(settings.get("commandlog") != null) {
            TextChannel textChannel = e.getGuild().getTextChannelById(settings.get("commandlog"));
        ...
        if(settings.get("moderationlog") != null) {
            TextChannel textChannel = e.getGuild().getTextChannelById(settings.get("moderationlog"));
        ...
        if(settings.get("eventchannel") != null) {
            TextChannel textChannel = e.getGuild().getTextChannelById(settings.get("eventchannel"));
        ...
        EmbedBuilder embed = new EmbedBuilder()
                ...
                .addField(e.getPrefix() + "language <string>", "`" + settings.get("language") + "` - The language used when displaying output.", false)
                .addField(e.getPrefix() + "prefix <string>", "`" + settings.get("prefix") + "` - The message prefix used to symbolise a command.", false)
                .addField(e.getPrefix() + "cleanupcommands <boolean>", "`" + settings.get("cleanupcommands") + "` - Deletes the users command when it is executed.", false)
                .addField(e.getPrefix() + "playnotifications <boolean>", "`" + settings.get("playnotifications") + "` - Sends information of the mew track when it changes.", false)
                .addField(e.getPrefix() + "djmode <boolean>", "`" + settings.get("djmode") + "` - Defines if DJ mode is on, meaning only users with the role 'DJ' can use certain audio commands.", false)
