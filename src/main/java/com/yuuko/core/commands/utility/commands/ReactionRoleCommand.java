...
public class ReactionRoleCommand extends Command {
    ...
    public void onCommand(MessageEvent e) {
        String[] parameters = e.getParameters().split("\\s+");
        Role highestSelfRole = e.getGuild().getSelfMember().getRoles().get(0); // Role list is ordered from highest to lowest
        ...
        final Message message = e.getChannel().retrieveMessageById(parameters[0]).complete();
